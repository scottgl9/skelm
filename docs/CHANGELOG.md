...
- **Persistent agents (`persistentAgent()`).** A long-lived conversational entity (sibling to `pipeline()`) whose identity and conversation outlive any single trigger fire. Each fire runs one bounded, gateway-enforced, audited turn that loads/saves a durable per-session conversation (keyed by a `sessionKey` you supply, e.g. a Telegram `chatId`) — surviving across messages and gateway restarts, with no resident process. Triggers (queue/cron/webhook) drive turns. Durable sessions reuse the run store's `StateStore` (`loadSession`/`saveSession`/`createSessionRecord`, `PersistentSessionRecord`). See [`docs/concepts/persistent-agents.md`](./concepts/persistent-agents.md) and the [Telegram persistent-agent recipe](./recipes/telegram-persistent-agent.md).
...
- **`examples/telegram-assistant/`** — a persistent, agentmemory-wired, unrestricted Telegram assistant demonstrating all of the above behind a who-can-talk allowlist.
