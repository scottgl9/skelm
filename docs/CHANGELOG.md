...
- **Backend wiring** in `@skelm/agent` (per-tool observe + smart-search recall under a `<memory>` block), `@skelm/codex`, `@skelm/opencode`, `@skelm/vercel-ai`, and `@skelm/pi` (per-turn observe + recall prepended to system prompts). Every backend now also captures the user prompt as a `user_prompt_submit` observation.
