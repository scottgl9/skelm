...
export class FileWatchTrigger {
  ...
  private async exists(path: string): Promise<boolean> {
    try {
      ...
      // Some Linux fs.watch rename events on a watched file path report the
      // parent filename rather than the full file path; retry the root path
      // when the basename matches.
