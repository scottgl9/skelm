...
/**
 * Inputs to {@link buildSystemPrompt}. All dynamic context the prompt needs
 * is passed in — the builder does no I/O.
 */
export interface SystemPromptInput {
  ...
  model?: string
  /** All tools the model can call this turn (built-ins + MCP merged). */
   ...
   * `replace` drops sections 1–9 and uses only agentDef + userSystem.
   */
  mode?: 'extend' | 'replace'
...
/** Hard ceiling on the built-in default (sections 1–9), guarded by tests. */
...
/** First sentence (or first 160 chars) of a tool/skill description. Keeps the prompt tight. */
...
// ---------------------------------------------------------------------------
// Backward-compatible adapter for callers that have an AgentRequest in hand.
// ---------------------------------------------------------------------------

/**
 * Build a system prompt from an {@link AgentRequest}. Convenience wrapper for backends.
 */
