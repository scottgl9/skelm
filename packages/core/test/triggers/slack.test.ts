  ...
  describe('initialization', () => {
    ...
    it('throws error for missing signing secret', async () => {
      ...
      await expect(trigger.initialize({
        id: 'slack-missing',
        botToken: 'xoxb-test-token',
      } as unknown as TriggerConfig)).rejects.toThrow('Slack trigger requires signingSecret')
    })
    ...
    it('throws error for missing bot token', async () => {
      ...
      await expect(trigger.initialize({
        id: 'slack-notoken',
        signingSecret: 'test-secret',
      } as unknown as TriggerConfig)).rejects.toThrow('Slack trigger requires botToken')
    })
  })
