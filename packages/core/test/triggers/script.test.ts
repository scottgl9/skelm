  ...
  describe('initialization', () => {
    ...
    it('throws error if command is missing', async () => {
      await expect(trigger.initialize({
        id: 'test-script',
        args: [],
      } as unknown as TriggerConfig)).rejects.toThrow('Script trigger requires a command')
    })
    ...
    it('throws error if args is missing', async () => {
      await expect(trigger.initialize({
        id: 'test-script',
        command: 'echo',
      } as unknown as TriggerConfig)).rejects.toThrow('Script trigger requires args array')
    })
