  ...
  describe('initialization', () => {
    ...
    it('throws error for missing homeserverUrl', async () => {
      ...
      await expect(trigger.initialize({
        ...
        userId: '@test:matrix.org',
        roomIds: ['!test:matrix.org'],
      } as unknown as TriggerConfig)).rejects.toThrow('Matrix trigger requires homeserverUrl')
    })
    ...
    it('throws error for missing access token', async () => {
      ...
      await expect(trigger.initialize({
        ...
        userId: '@test:matrix.org',
        roomIds: ['!test:matrix.org'],
      } as unknown as TriggerConfig)).rejects.toThrow('Matrix trigger requires accessToken')
    })
    ...
    it('throws error for missing room IDs', async () => {
      ...
      await expect(trigger.initialize({
        ...
        accessToken: 'test-token',
        userId: '@test:matrix.org',
      } as unknown as TriggerConfig)).rejects.toThrow('Matrix trigger requires at least one roomId')
    })
  })
