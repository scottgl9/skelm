...
class MockWorkflowPlugin extends WorkflowPluginBase {
  ...
  constructor(
    ...
    this.mockHealthCheck = healthCheck || (() => Promise.resolve({
      healthy: true,
      status: 'healthy',
    }))
  }
...
describe('WorkflowRegistry', () => {
  ...
  describe('register/unregister', () => {
    ...
    it('stops active workflow before unregistering', async () => {
      const mockWorkflow = new MockWorkflowPlugin('stop-before-unregister', 'Stop Before Unregister')
  ...
  describe('health check', () => {
    it('checks health of all workflows', async () => {
      const mockWorkflow = new MockWorkflowPlugin('health-workflow', 'Health Workflow', '1.0.0', undefined, undefined, () => 
      ...
      expect(health['health-workflow']).toEqual({ healthy: true, status: 'healthy', lastCheck: expect.any(Date) })
    })
    
    it('returns error status for unhealthy workflows', async () => {
      const mockWorkflow = new MockWorkflowPlugin('unhealthy-workflow', 'Unhealthy Workflow', '1.0.0', undefined, undefined, () => 
    ...
    it('returns healthy workflows list', async () => {
      const healthyWorkflow = new MockWorkflowPlugin('healthy', 'Healthy', '1.0.0', undefined, undefined, () => 
        Promise.resolve({ healthy: true, status: 'healthy' })
      )
      const unhealthyWorkflow = new MockWorkflowPlugin('unhealthy', 'Unhealthy', '1.0.0', undefined, undefined, () => 
