...
function handle(message: Json): void {
  ...
  if (method === 'initialize') {
    reply(id, {
      ...
      serverInfo: { name: 'mock-shell-mcp', version: '0.1.0' },
    })
    return
  }
  ...
  if (method === 'tools/call') {
    const params = message.params as { arguments?: { command?: string } } | undefined
    reply(id, {
      content: [
        {
          type: 'text',
          text: `exec:${params?.arguments?.command ?? ''}`,
        },
      ],
      isError: false,
    })
  ...
  while (true) {
    ...
    try {
      ...
      if (Array.isArray(parsed)) {
        for (const message of parsed) handle(message as Json)
      } else {
        handle(parsed as Json)
      }
    } catch {
      // Ignore malformed test input in the mock process.
    }
  }
})
