...
describe('Context.get<T>', () => {
  ...
  it('preserves the unknown fallback on ctx.steps so existing call sites still compile', () => {
    ...
    expectTypeOf<Ctx['assets']['getText']>().toBeFunction()
    expectTypeOf<ReturnType<Ctx['assets']['getBytes']>>().toEqualTypeOf<Promise<Uint8Array>>()
    expectTypeOf<Ctx>().toHaveProperty('get')
    expectTypeOf<Ctx>().toHaveProperty('assets')
  })
})
