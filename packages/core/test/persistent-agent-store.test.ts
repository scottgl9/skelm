...
import { createSessionRecord, loadSession, saveSession } from '../src/persistent-agent-store.js'
...
describe('persistent-agent session store', () => {
  for (const [label, store] of stores()) {
    describe(label, () => {
      it('returns undefined for an unknown session', async () => {
        await expect(loadSession(store, 'agent-x', 'chat-1')).resolves.toBeUndefined()
      })

      it('round-trips a record and keeps the sessionId stable across saves', async () => {
        const rec = createSessionRecord('agent-1', 'chat-1')
        await saveSession(store, rec)

        const loaded = await loadSession(store, 'agent-1', 'chat-1')
        ...
        const reloaded = await loadSession(store, 'agent-1', 'chat-1')
      ...
      it('isolates sessions by (agentId, sessionKey)', async () => {
        const a = createSessionRecord('agent-1', 'chat-A')
        const b = createSessionRecord('agent-1', 'chat-B')
        await saveSession(store, a)
        await saveSession(store, b)
        expect(a.sessionId).not.toBe(b.sessionId)
        expect((await loadSession(store, 'agent-1', 'chat-A'))?.sessionId).toBe(a.sessionId)
        expect((await loadSession(store, 'agent-1', 'chat-B'))?.sessionId).toBe(b.sessionId)
      })
    })
  }
})
