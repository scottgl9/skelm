...
import type { TriggerConfig, TriggerType, TriggerEvent, TriggerEventHandler, TriggerHealthStatus } from './types.js'
...
export abstract class TriggerPluginBase {
  ...
  async initialize(config: TriggerConfig): Promise<void> {
    ...
    try {
      ...
      this.logger.error(`Failed to initialize trigger: ${error instanceof Error ? error.message : String(error)}`)
      throw error
    }
  }
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      this.state = TriggerState.ERROR
      this.logger.error(`Failed to start trigger: ${error instanceof Error ? error.message : String(error)}`)
      throw new TriggerStartError(`Failed to start trigger: ${error instanceof Error ? error.message : String(error)}`, error)
    }
  }
  ...
  async stop(): Promise<void> {
    ...
    try {
      ...
      this.state = TriggerState.ERROR
      this.logger.error(`Failed to stop trigger: ${error instanceof Error ? error.message : String(error)}`)
      throw new TriggerStopError(`Failed to stop trigger: ${error instanceof Error ? error.message : String(error)}`, error)
    }
  }
  ...
  protected async emitEvent(event: TriggerEvent): Promise<void> {
    ...
    const promises = this.handlers.map(async (handler) => {
      try {
        ...
        this.logger.error(`Handler error for event ${event.eventId}: ${error instanceof Error ? error.message : String(error)}`)
      }
    })
