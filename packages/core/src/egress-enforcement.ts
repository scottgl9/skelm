...
/**
 * In-process backends (vercel-ai, pi-sdk, …) cannot intercept globalThis.fetch
 ...
 *
 * Subprocess backends (pi RPC, opencode) do not call this — the gateway egress
...
export function assertEgressEnforceable(
  ...
  throw new PermissionDeniedError(
    `${backendName} backend cannot enforce networkEgress in-process. Set networkEgress: "allow" for this step, or use a subprocess backend (pi RPC, opencode) so the gateway egress proxy can intercept outbound traffic.`,
  )
}
